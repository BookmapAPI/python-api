...
def handle_instrument_info(addon, alias, full_name, is_crypto, pips, size_multiplier, instrument_multiplier):
    ...
    bm.register_indicator(addon, alias, req_id, "Bid sum", "BOTTOM", color=(204, 204, 0))
    req_id+=1
    request_id_to_related_indicator_alias[req_id] = (alias, False)
    bm.register_indicator(addon, alias, req_id, "Ask sum", "BOTTOM", color=(0, 0, 204))
    req_id+=1
    bm.subscribe_to_depth(addon, alias, req_id)
    liquidity_sizes[alias] = DEFAULT_LIQUIDITY_SIZE
    bm.add_number_settings_parameter(addon, alias, "Liquidity size", DEFAULT_LIQUIDITY_SIZE, 5, 25, 1)
    ...
    for alias, order_book in alias_to_orber_book.items():
        if alias in alias_to_ask_liquidity_indicator and alias in alias_to_bid_liquidity_indicator:
            ...
            if alias in alias_to_instrument and alias in liquidity_sizes:
                ...
                bid_liquidity_in_ticks, ask_liquidity_in_ticks = bm.get_sum(order_book, liquidity_size)
                bm.add_point(addon, alias, ask_liquidity_indicator, float(ask_liquidity_in_ticks) / instrument_size_multiplier)
                bm.add_point(addon, alias, bid_liquidity_indicator, float(bid_liquidity_in_ticks) / instrument_size_multiplier)
...
def on_settings_change_handler(addon, alias: str, setting_name: str, field_type: str, new_value):
    global liquidity_sizes
    print("Received settings changed " + str(alias) + " " + str(setting_name) + " " + str(field_type) + " " + str(new_value), flush=True)
